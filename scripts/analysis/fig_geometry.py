"""Winner dictionary-geometry figures (2D set).

Structural geometry of the current winner (data/phase0/winner.json) and
...
analysis dir:
  ...
  geo_share.png       per-block depth-energy share, winner vs primary,
                      + depth-argmax histograms
  geo_rotation.png    adjacent-site frame rotation by depth (both arms +
                      shuffled-block null) and stream-vs-frame tracking
                      for the qualified showcase blocks
  geo_dimensions.png  cross-site stacked spectral dimension (is a block
                      one rotating subspace or S fresh ones)
  geo_packing.png     co-activation Jaccard structure, clique membership,
                      code anisotropy per arm
  census.png          shape-space census: every sane-frequency block by
                      code PR / top-2 mass, qualified showcase manifolds
                      highlighted

plus geometry_summary.json beside the inputs. Block identities come from
showcase_blocks.json — the pilot's hand-curated identity/oddball catalog
(and its scalar/destroyed-arm panels) lives verbatim in
scripts/archive/fig_geometry4b.py.
...
def fig_rotation(summary, show):
    ...
    if panels:
        ...
        if "month" in act_fams:
            zc_any = np.load(DATA / f"block_codes_{W['run_name']}.npz")
    ...
    fig.savefig(OUT / "geo_rotation.png")
    plt.close(fig)
...
def fig_packing(summary):
    ...
    fig.savefig(OUT / "geo_packing.png")
    plt.close(fig)
