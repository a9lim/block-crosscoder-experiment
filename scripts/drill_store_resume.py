"""Tranche 7 (runbook-phase099): interrupted-harvest recovery drill.
...
4. prints the resume rule: a resumed harvest must write a NEW split and
   merge manifests (the extend_store.py pattern) — a fresh ``ShardWriter``
   restarts shard numbering at 0, so in-place append would collide.

  python -u scripts/drill_store_resume.py
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--scratch", type=Path,
        default=Path("/data/runs/bcc-phase099/drill_store"),
    )
    ...
    writer = ShardWriter(
        ...
        meta={"drill": "tranche7 resume rehearsal"},
    ...
    print(
        ...
        "+ margin, then merge manifests (extend_store.py train12m pattern).",
        flush=True,
    )
