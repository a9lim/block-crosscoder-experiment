"""Evaluate consolidation and order for every descriptive zoo family.

For every family in the captured activation artifact:
...
Two GPU passes per run keep the full score tensor out of host memory:
pass 1 accumulates class-mean scores, pass 2 collects the selected
blocks' codes and writes a compact Mac-side export.

Defaults follow ``data/winner.json`` and emit winner-scoped artifacts.
"""
...
from block_crosscoder_experiment.analysis.catalog import FAMILIES
from block_crosscoder_experiment.store import Whitener

from .geography import COUNTRY_GEO
from .metrics import ring_permutation_p, ring_stats
...
def main() -> None:
    ...
    args = ap.parse_args()
    args.acts = args.acts or adir / "zoo_activations.npz"
    args.store = args.store or Path(winner["store"])
    args.tokenizer = args.tokenizer or winner["model"]
    args.out_dir = args.out_dir or adir
    ...
    runs = (
        ...
        else dict(pair.split("=", 1) for pair in args.runs)
    )
    ...
    # Stream-space means must use the exact same token filter as capture and
    # order statistics; otherwise lowercase "may" silently corrupts the month
    # display while the reported gate remains capitalization-only.
    stream_means = {}
    acts_np = acts.numpy()
    for fi, family in enumerate(families):
        mask = family_mask(fi, family)
        class_ids = cls[mask]
        family_acts = acts_np[mask]
        stream_means[family] = np.stack(
            [
                family_acts[class_ids == class_index].mean(0)
                if np.any(class_ids == class_index)
                else np.zeros(acts_np.shape[1:], dtype=np.float32)
                for class_index in range(len(FAMILIES[family]))
            ]
        )
    np.savez_compressed(
        args.out_dir / f"zoo_means{args.tag}.npz",
        **{f"{family}_means": means for family, means in stream_means.items()},
        meta=json.dumps({**meta, "capitalization_filtered": sorted(CAP_FAMILIES)}),
    )
        ...
        for fi, family in enumerate(families):
            ...
            if family in GEO:
                ...
                e["order"] = {"kind": "ring", "hits": hits, "max": n_ring,
                              ...
                              "perm_p": ring_permutation_p(hits, n_ring, args.perms)}
            else:
