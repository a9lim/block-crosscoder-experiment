"""The Phase 0.9.9 sealed probe panel — DO NOT SCAN BEFORE UNSEALING.

Six concept families preregistered blind (runbook-phase099 tranche 0,
a9-ratified 2026-07-18): fixtures written from world knowledge only, with
...
unsealing budgeted for the tranche-5 lr re-ratification bar. Unsealing is
...
def assert_unsealed() -> None:
    ...
    if os.environ.get(_UNSEAL_VAR) != "1":
        raise PanelSealedError(
            "The 0.9.9 probe panel is SEALED (runbook-phase099 tranche 0). "
...
def build_sealed_label_map(tokenizer, family: str) -> dict[int, int]:
    ...
    assert_unsealed()
    from .labels import _surface_forms

    words = SEALED_FAMILIES[family]
    mapping: dict[int, int] = {}
    for k, word in enumerate(words):
        for form in dict.fromkeys(
            _surface_forms(word, family in SEALED_CAP_ONLY)
        ):
